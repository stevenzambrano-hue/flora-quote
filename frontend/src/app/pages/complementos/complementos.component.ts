      ...
      @if (isEditing()) {
        <div class="max-w-2xl bg-white p-10 rounded-3xl shadow-xl border border-slate-100">
           <h3 class="text-2xl font-bold text-slate-900 mb-8 border-l-4 border-indigo-600 pl-4">
             {{ editMode() === 'create' ? 'Add New Accessory' : 'Update Accessory' }}
           </h3>

           <form [formGroup]="form" (ngSubmit)="save()" class="grid grid-cols-2 gap-6">
              <div class="col-span-2">
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Accessory Name</label>
                <input formControlName="nombre" class="w-full bg-slate-50 border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500 transition-all border border-slate-200" placeholder="e.g. Card"/>
              </div>
